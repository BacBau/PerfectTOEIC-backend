...
public class ErrorUpdateTempStorage {
    private static final Logger logger = LoggerFactory.getLogger(ErrorUpdateTempStorage.class);
    protected Class<?> clazz;
    protected File updateDir;
    protected File errorDir;

    public ErrorUpdateTempStorage() {
    }

    public void setEnviroment(Environment environment, String schemaName) {
        this.updateDir = this.getDirectory(environment, schemaName, "update");
        if (!this.updateDir.exists()) {
            this.updateDir.mkdirs();
        }

        this.errorDir = this.getDirectory(environment, schemaName, "update_error");
    ...
    public void saveTemp(DataContainer<UpdateField<?>> bean) {
        try {
            File file = null;
            if (bean.getErrorTime() > LazyRepositoryImpl.MAX_TIME_TRY_SAVE) {
                file = new File(this.errorDir, ((UpdateField)bean.getValue()).toUniqueIdentifier() + ".json");
            } else {
                file = new File(this.updateDir, ((UpdateField)bean.getValue()).toUniqueIdentifier() + ".json");
            }
    ...
    public List<DataContainer<UpdateField<?>>> loadFromTemp() {
        File[] files = this.updateDir.listFiles((f) -> {
        ...
        if (files != null && files.length >= 1) {
            ...
            List<DataContainer<UpdateField<?>>> list = new ArrayList(110);
            File[] var4 = files;
            ...
            for(int var6 = 0; var6 < var5; ++var6) {
                ...
                if (!file.isDirectory()) {
                    try {
                        ...
                        TypeReference<DataContainer<UpdateField<?>>> typeReference = new TypeReference<DataContainer<UpdateField<?>>>() {
                        };
